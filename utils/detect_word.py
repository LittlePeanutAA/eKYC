...
# Crop image
def perspective_transform(image, source_points, point):
    x = point[0]
    y = point[1]
...
# OCR
def OCR(result, cls_name, img, detector):
    """
    result = results.xyxy[i] : bounding box at information
    result_pandas = results.pandas().xyxy[i]
    img = imgs[i] : cropped image
    cls_name: name of information classes
    detector: VietOCR detector
    """
    if len(result) == 0:  # Remove case: Nothing found
        return None
    ...
    # Label
    label = [cls_name[i] for i in result.cls.numpy()]

    # 2. Overlap
    final_boxes, final_labels = non_max_suppression_fast(tensor.numpy(), label, 0.3)
    # print(final_boxes)
    # print(final_labels)
    dic = dict()  # Save information on the card
    y = 0  # Save max-y position

    # 3. Crop images in boxes
    for i in range(len(final_boxes)):
        ...
        source_points = np.float32([[x_min, y_min],
                                    [x_max, y_min],
                                    [x_min, y_max],
                                    [x_max, y_max]])

        new_img = perspective_transform(img, source_points, point)  # Cat anh cua tung box
        # print(final_labels[i])

        # 4. Image Processing
        new_img = Img_Processing(new_img)

        # 5. Predict information
